        ...
        struct virtio_blk_zone_descriptor desc =
            (struct virtio_blk_zone_descriptor) {
                ...
                .z_wp = cpu_to_le64(data->zone_report_data.zones[j].wp
                    >> BDRV_SECTOR_BITS),
                /*
                 * Linux virtio-blk driver implicitly converts QEMU constants
                 * back to Linux macros under the assumption that the actual
                 * integer values for both sets of definitions in Linux
                 * virtio-blk driver and QEMU constants are the same.
                 */
...
static void virtio_blk_zone_append_complete(void *opaque, int ret)
{
    ...
    iov_discard_undo(&req->inhdr_undo);
    n = iov_from_buf(in_iov, in_num, 0, &zap_hdr, sizeof(zap_hdr));
    if (n != sizeof(zap_hdr)) {
        virtio_error(vdev, "Driver provided input buffer less than size of "
                           "zone_append in header");
    }
    goto out;
