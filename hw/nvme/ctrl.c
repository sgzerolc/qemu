...
//            trace_pci_nvme_err_write_not_at_wp(slba, zone->d.start,
//                                               zone->w_ptr);
//            return NVME_ZONE_INVALID_WRITE;
//        }
//    }
...
//        return NVME_SUCCESS;
//    case NVME_ZONE_STATE_OFFLINE:
//        trace_pci_nvme_err_zone_is_offline(zone->d.start);
//        return NVME_ZONE_OFFLINE;
//    default:
//        assert(false);
//    }
...
//
//    trace_pci_nvme_zoned_zrwa_implicit_flush(zone->d.start, nlbc);
//
//    zone->w_ptr += nlbc;
//
...
//            }
            ...
            if (unlikely(slba != zone->d.start)) {
                trace_pci_nvme_err_append_not_at_start(slba, zone->d.start);
            ...
            case NVME_ID_NS_DPS_TYPE_2:
                if (piremap) {
                    uint32_t reftag = le32_to_cpu(rw->reftag);
                    rw->reftag = cpu_to_le32(reftag + (slba - zone->d.start));
                }
...
//    }
//
//    moff = nvme_moff(ns, iocb->zone->d.start);
...
//        }
//
//        trace_pci_nvme_zns_zone_reset(zone->d.start);
//
//        iocb->aiocb = blk_aio_pwrite_zeroes(ns->blkconf.blk,
//                                            nvme_l2b(ns, zone->d.start),
...
//    uint16_t err_status;
    ...
    for (; j < nz; ++j) {
        ...
        switch (iocb->zone_report_data.zones[j].type) {
        case BLK_ZT_CONV:
            z->zt = NVME_ZONE_TYPE_RESERVED;
            break;
        case BLK_ZT_SWR:
            z->zt = NVME_ZONE_TYPE_SEQ_WRITE;
            break;
        case BLK_ZT_SWP:
            z->zt = NVME_ZONE_TYPE_RESERVED;
            break;
