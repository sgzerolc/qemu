                              ...
                              bool wrz)
{
    ...
    if (blk_get_zone_macro(ns->blkconf.blk, "zoned")) {
        assert(wps);
...
static inline uint8_t *nvme_get_zd_extension(NvmeNamespace *ns,
                                        uint32_t zone_idx)
{
    return &ns->zd_extensions[zone_idx * blk_get_zone_info(ns->blkconf.blk,
                                                   "zd_ext_size")];
}
...
static uint16_t nvme_zone_mgmt_send(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    if (op != BLK_ZO_INVALID) {
        ...
        blk_aio_zone_mgmt(blk, op, offset, blk_get_zone_info(blk, "zone_size"),
...
static uint16_t nvme_zone_mgmt_recv(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    if (zra == NVME_ZONE_REPORT_EXTENDED && !blk_get_zone_info(ns->blkconf.blk, "zd_ext_size")){
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    ...
    if (zra == NVME_ZONE_REPORT_EXTENDED) {
        zone_entry_sz += blk_get_zone_info(ns->blkconf.blk, "zd_ext_size") ;
    }
    ...
    if (!partial) {
        nr_zones = blk_get_zone_info(blk, "nr_zones");
        offset = 0;
    }
