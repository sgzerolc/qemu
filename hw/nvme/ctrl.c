...
static uint16_t nvme_zone_mgmt_send(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    case NVME_ZONE_ACTION_SET_ZD_EXT:
        ...
        if (all || !blk_get_zone_info(ns->blkconf.blk, "zd_ext_size")) {
            return NVME_INVALID_FIELD | NVME_DNR;
        }
        zd_ext = nvme_get_zd_extension(ns, zone_idx);
        status = nvme_h2c(n, zd_ext, blk_get_zone_info(ns->blkconf.blk, "zd_ext_size"), req);
        if (status) {
...
static uint16_t nvme_zone_mgmt_recv(NvmeCtrl *n, NvmeRequest *req)
{
    ...
    for (; zone_idx < ns->num_zones && max_zones > 0; zone_idx++) {
        ...
        if (nvme_zone_matches_filter(zrasf, zone)) {
            ...
            if (zra == NVME_ZONE_REPORT_EXTENDED) {
                int zd_ext_size = blk_get_zone_info(ns->blkconf.blk, "zd_ext_size");
