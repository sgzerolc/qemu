...
static int nvme_ns_zoned_check_calc_geometry(NvmeNamespace *ns, Error **errp)
{
    ...
    uint64_t zone_size = blk_get_zone_info(blk, "zone_size");
    uint64_t zone_cap = blk_get_zone_info(blk, "zone_capacity");
...
static void nvme_ns_zoned_init_state(NvmeNamespace *ns)
{
    ...
    if (blk_get_zone_info(blk, "zd_ext_size")) {
...
static void nvme_ns_init_zoned(NvmeNamespace *ns)
{
    ...
    ns->zone_size = blk_get_zone_info(blk, "zone_size");

    /* MAR/MOR are zeroes-based, FFFFFFFFFh means no limit */
    id_ns_z->mar = cpu_to_le32(blk_get_zone_info(blk, "mar") - 1);
    id_ns_z->mor = cpu_to_le32(blk_get_zone_info(blk, "mor") - 1);
    id_ns_z->zoc = 0;
    ...
    for (i = 0; i <= ns->id_ns.nlbaf; i++) {
        ...
        id_ns_z->lbafe[i].zdes =
            blk_get_zone_info(blk, "zd_ext_size") >> 6; /* Units of 64B */
    }
...
static int nvme_ns_check_constraints(NvmeNamespace *ns, Error **errp)
{
    ...
    if (blk_get_zone_macro(blk, "zone_profile") == BLK_ZP_ZNS && ns->endgrp
    && ns->endgrp->fdp.enabled) {
        error_setg(errp, "cannot be a zoned- in an FDP configuration");
        return -1;
    }

    if (blk_get_zone_macro(blk, "zoned")) {
        if (ns->params.zrwas) {
            ...
            int maz = blk_get_zone_info(blk, "maz");
            if (maz) {
...
int nvme_ns_setup(NvmeNamespace *ns, Error **errp)
{
    ...
    if (blk_get_zone_macro(ns->blkconf.blk, "zone")) {
